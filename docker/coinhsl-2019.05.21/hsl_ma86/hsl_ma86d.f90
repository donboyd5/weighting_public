...
! * Change wp
! * Change _double
! * Change BLAS calls: dgemv, dgemm, dtrsm, dtrsv, dcopy, daxpy, dswap
! * Change HSL calls: mc77id, mc77ad, mc64wd
...
!*************************************************
   ...
   interface MA86_analyse
      module procedure MA86_analyse_double
   end interface

   interface MA86_factor
      module procedure MA86_factor_double
   end interface

   interface MA86_factor_solve
      module procedure MA86_factor_solve_one_double, &
                       MA86_factor_solve_mult_double
   end interface

   interface MA86_solve
      module procedure MA86_solve_one_double, &
         MA86_solve_mult_double
   end interface
...
!$    integer(omp_lock_kind) :: alock  ! Lock for altering values in keep%lfact 
   ...
   type lfactor
      integer :: blkn_new ! number of columns in block column incl delays
      ...
      integer :: nrow ! number of rows in block column at start of factorize
         ! ie number of rows in lcol. Number of rows after delays is
...
!
subroutine MA86_analyse_double(n, ptr, row, order, keep, control, info)
   ...
   ! Local arrays
   integer(long), allocatable :: amap(:) ! map from user a to reordered a
...
!**integer :: t_start, t_end, t_rate
   ...
   do node = 1, num_nodes
      ...
      if (st /= 0) go to 490
      j = 1
      do ilong = rptr(node), rptr(node+1)-1
         keep%nodes(node)%index(j) = rlist(ilong)
         j = j + 1
      end do
      ...
      keep%nodes(node)%blk_en = keep%nodes(node-1)%blk_en + jlong
...
!**call system_clock(t_start)
   ...
   do node = 1, num_nodes
      ...
      do ci = sa, en, l_nb
         klong = 1 ! use klong to hold position of block within block column
...
!$          call omp_init_lock(keep%blocks(blk)%alock)
            ...
            klong = klong + keep%blocks(blk)%blkm * &
               (keep%blocks(blk)%blkn+0_long)
...
end subroutine MA86_analyse_double
...
subroutine make_map(n, perm, optr, orow, nptr, nrow, map)
   ...
   integer(long), dimension(optr(n+1)-1), intent(out) :: map
   ...
   do i = 1, n
      ...
      do j = optr(i), optr(i+1)-1
         k = perm(orow(j))
         if(k<l) then
            ! in upper triangle of reordered matrix
            map(nptr(k+1)) = j
            nrow(nptr(k+1)) = l
            nptr(k+1) = nptr(k+1) + 1
         else
...
subroutine lcol_map(aptr, arow, num_nodes, nodes, blocks, lmap, map, amap, st)
   ...
   integer(long), dimension(:), intent(in) :: amap ! map set up by make_map
   ...
   do snode = 1, num_nodes ! loop over nodes
      ...
      do cb = sa, en, l_nb
         ...
         offset = blocks(dblk)%sa - (cb-sa+0_long)*blocks(dblk)%blkn
         ...
         if(nodes(snode)%nchild.eq.0 .and. cb.eq.sa) then
            ...
            do col = cb, min(cb+l_nb-1, en)
               ...
               do jlong = aptr(col), aptr(col+1)-1
                  i = arow(jlong)
                  i = map(i)
                  i = offset + i
                  lmap(bcol)%map(1,k) = i ! destination in lfact(:)%lcol
                  lmap(bcol)%map(2,k) = amap(jlong) ! source
                  k = k + 1
               end do
               offset = offset + nrow
            end do
...
! Factorisation phase.
!
subroutine MA86_factor_double(n, ptr, row, val, order, &
      keep, control, info, scale)
   integer, intent(in) :: n ! order of A
   integer, intent(in) :: row(:) ! row indices of lower triangular part
   integer, intent(in) :: ptr(:) ! col pointers for lower triangular part
   real(wp), intent(in) :: val(:) ! matrix values for lower triangular part
...
!**integer :: t_start, t_end, t_rate
   ...
   if(control%static.lt.abs(control%small) .and. control%static.ne.zero) then
...
!**call system_clock(t_start)
   ...
   if(allocated(keep%scaling)) then
      call factorize_indef(val, order, keep, control, info, &
...
end subroutine MA86_factor_double
...
!
subroutine MA86_factor_solve_one_double(n, ptr, row, &
   ...
   call MA86_factor_solve_mult_double(n, ptr, row, &
      val, order, keep, control, info, 1, keep%n, x, scale=scale)

end subroutine MA86_factor_solve_one_double
...
!
subroutine MA86_factor_solve_mult_double(n, ptr, row, &
...
!**end if
   ...
   call solve_indef(SOLVE_JOB_D_AND_BWD, nrhs, soln, n, keep, &
      control, info)
...
end subroutine MA86_factor_solve_mult_double
...
!
subroutine MA86_solve_one_double(x,order,keep,control,info,job,scale)
   ...
   call MA86_solve_mult_double(1, keep%n, x, order, keep, &
      control, info, job)

end subroutine MA86_solve_one_double
...
!
subroutine MA86_solve_mult_double(nrhs,lx,x,order,keep, &
   ...
   integer :: st ! stat parameter
   real(wp), dimension(:), allocatable :: soln ! allocated to have size
   ...
   call solve_indef(local_job, nrhs, soln, n, keep, control, &
      info)
...
end subroutine MA86_solve_mult_double
...
! in the package.
!
subroutine MA86_finalise_double(keep, control)
...
end subroutine MA86_finalise_double
...
subroutine mc64_scale(n, ptr, row, val, scaling, control, flag, st)
   ...
   call mc64wd(n,ptr2(n+1)-1,ptr2,row2,val2,cperm,ndiag, &
      iw(1),iw(n+1),iw(2*n+1),iw(3*n+1),iw(4*n+1), &
      dw(1),dw(n+1))
   if (ndiag.eq.n) then
      do i = 1,n
         if (dw(2*n+i).ne.zero) then
...
subroutine mc77_scale(n, ptr, row, val, scaling, st)
   ...
   call mc77ad(0, n, n, ptr(n+1)-1, ptr, row, val2, iw, size(iw), &
...
!
subroutine factorize_indef(val, order, keep, control, &
   ...
   integer :: sa ! holds keep%nodes(snode)%sa
   integer(long) :: size_bcol ! number of entries in the block column (sum over
      ! the row blocks in the column)
   integer :: snode 
   integer :: st ! stat parameter
...
!$   end do
   ...
   do snode = 1, nnodes
      ...
      do i = sa, en, l_nb
         ...
         size_bcol = (nrow+0_long)*keep%blocks(dblk)%blkn
...
!$       call omp_init_lock(keep%lfact(nbcol)%lock)
   ...
   do ilong = 1,keep%final_blk
     bcol = keep%blocks(ilong)%bcol
     if (keep%blocks(ilong)%id.eq.keep%blocks(ilong)%dblk) then
       keep%lfact(bcol)%dep = keep%lfact(bcol)%dep + &
          keep%blocks(ilong)%dep_initial
     else
       ! for off diag. blocks, reduce count by one since no solve
       ! for combined factor-solve task
       keep%lfact(bcol)%dep = keep%lfact(bcol)%dep + &
         keep%blocks(ilong)%dep_initial - 1
     end if
     ! flag block as not yet touched
     keep%blocks(ilong)%touched = .false.
   end do
...
!**call system_clock(t_start, t_rate)
   ...
   rhs_local(1:nrhs*ldr,0:total_threads-1) = zero
...
!$OMP SHARED(control,info,invp,keep,ldr,nnodes,nrhs,order,rhs, &
...
!**!$OMP END SINGLE NOWAIT
      ...
      call task_dispatch(val, invp, keep%nbcol, keep%maxm, &
...
!
subroutine blk_col_add_a(local, val, lmap, invp, snode, dblk, lcol, scale)
   ...
   real(wp), dimension(*), intent(inout) :: lcol
   real(wp), dimension(*), optional, intent(in) :: scale

   integer :: k, jcol, swidth
   integer(long) :: i, j, offset, offset2

   swidth = dblk%blkn
   offset = snode%sa + (local-1_long)*snode%nb
   offset2 = (local-1_long)*snode%nb
   ...
   if(present(scale)) then
      do i = 1, lmap%len_map
         ...
         jcol = invp( offset + int(mod(j-1,swidth+0_long)) ) ! col of original
         k = int((j-1)/swidth) + 1 ! row of local
         k = invp( snode%index(offset2+k) ) ! row of original
         lcol(j) = lcol(j) + scale(jcol) * val(lmap%map(2,i)) * scale(k)
      end do
   else
      do i = 1, lmap%len_map
         lcol(lmap%map(1,i)) = lcol(lmap%map(1,i)) + val(lmap%map(2,i))
      end do
   endif
...
!
subroutine blk_col_add_a_leaf(val, lmap, invp, snode, dblk, &
      colwork, blkm, scale)
   real(wp), dimension(*), intent(in) :: val
   type(lmap_type), intent(in) :: lmap ! mapping set up by analyse phase
   integer, dimension(*), intent(in) :: invp
   type(node_type), intent(in) :: snode
   type(block_type), intent(in) :: dblk
   real(wp), dimension(*), intent(out) :: colwork
   integer, intent(in) :: blkm
   real(wp), dimension(*), optional, intent(in) :: scale

   integer :: k, jcol, blkn
   integer(long) :: i, j, offset

   blkn = dblk%blkn
   offset = snode%sa

   colwork(1:blkn*blkm) = zero
   ...
   if(present(scale)) then
      do i = 1, lmap%len_map
         ...
         k = int(mod(j-1,blkm+0_long)) + 1 ! row of local
         k = invp( snode%index(k) ) ! row of original
         colwork(j) = colwork(j) + scale(jcol) * val(lmap%map(2,i)) * scale(k)
      end do
   else
      do i = 1, lmap%len_map
         j = lmap%map(1,i)
         colwork(j) = colwork(j) + val(lmap%map(2,i))
      end do
   endif
...
!
subroutine solve_indef(job, nrhs, rhs, ldr, &
      keep, control, info)
   ...
   real(wp), dimension(:), allocatable :: rhslocal ! allocated to have
      ! size (maxmn*nrhs).
...
!$OMP SHARED(control, counts, info, job, keep, ldr, maxmn, nrhs, &
!$OMP    rhs, rhs_local, st, stack, total_threads)
   this_thread = 0
!$ this_thread = omp_get_thread_num()

   call solve_task_dispatch(keep%nbcol, keep%lfact, stack,                    &
...
! Main worker routine
!
subroutine task_dispatch(val, invp, nbcol, maxm, maxn, lfact, &
   ...
   real(wp), dimension(:), allocatable :: buffer ! work array
   real(wp), dimension(:), allocatable :: colwork ! buffer for storing colwise 
   ...
   integer :: node 
   integer(long) :: sa ! set to blocks(blk)%sa 
   integer(long) :: sa1 ! set to blocks(dblk)%sa + s
   integer(long) :: sa2 ! set to blocks(task%src2)%sa + s
...
!$OMP FLUSH(lfact)
      ...
      case(TASK_FACTORIZE_COLUMN) ! Factorize and solve
         ...
         if(nodes(node)%nchild.eq.0 .and. lfact(bcol)%local.eq.1) then
            ...
            call blk_col_add_a_leaf(val, lmap(bcol), invp, nodes(node), &
               blocks(dblk), colwork, blkm, scale=scale)
            ...
            do blk = dblk, blocks(dblk)%last_blk
               ...
               n = blocks(blk)%blkn
               lfact(bcol)%lcol(blocks(blk)%sa:blocks(blk)%sa+m*n-1) = zero
               blocks(blk)%touched = .true.
            end do

            ! Add original matrix entries to updates accummulated in lcol
            call blk_col_add_a(lfact(bcol)%local, val, &
            ...
            call merge_delays(nodes, blocks, lfact, pos, dblk, blkm, blkn, &
               tstats, colwork, st)
            if (st.ne.0) then
               info = MA86_ERROR_ALLOCATION
               call MA86_print_flag(info, control, context='MA86_factor',st=st)
         ...
         ! Perform actual factorization
         call factor_solve_block(blkm, blkn, blkn, delay_col, &
         ...
         do i = 0, lfact(bcol)%nelim-1
            tstats%num_flops = tstats%num_flops + (blkm-i)**2_long
...
!   lfact(bcol)%index_new(1:blkn) = lfact(bcol)%index_new(map(1:blkn))
         ...
         if(nrhs.gt.0) then
            ...
            blk = dblk
            call fwd_solve_bcol(bcol, blk, 1, lfact, blocks, &
               nrhs, rhs, ldr, work, lfact(bcol)%nelim, xlocal, rhs_local, &
!%%%           control, &
               this_thread, total_threads)
         endif
...
!$OMP FLUSH 
      ...
      case(TASK_UPDATE_INTERNAL) ! update one block by two others
         ...
         if (nelim.gt.0) then
            if(size(work).lt.n1*(m1+0_long)) then !Resize work array for calc_ld
               deallocate(work)
               allocate(work(n1*(m1+0_long)),stat=st)
               if(st.ne.0) goto 100
            endif
            call update_block_block(m, n, nelim, &
               lfact(bcol)%lcol(sa:sa+n*(m+0_long)-1), blocks(blk), nelim, &
               lfact(bcol_src)%lcol(sa1:sa1+nelim*(m1+0_long)-1), &
               lfact(bcol_src)%lcol(sa2:sa2+nelim*(m2+0_long)-1), &
...
!%%%           , control &
      ...
      case(TASK_UPDATE_BETWEEN) ! update a block with information from 
         ...
         nelim  = lfact(bcol_src)%nelim
         if (nelim.gt.0) then
            call update_between(blk, nodes(blocks(blk)%node),              &
               n1, nelim, lfact(bcol_src)%index_new, lfact(bcol)%lcol,     &
               lfact(bcol_src)%lcol, lfact(bcol_src)%d,                    &
               blocks, col_list, row_list, buffer, control, info, st, work &
!%%%           slocal, nodes(blocks(task%src1)%node)                       &
               )
         endif
         if(info.lt.0) return
...
!
subroutine merge_delays(nodes, blocks, lfact, pos, dblk, blkm_new, &
   ...
   real(wp), dimension(:), allocatable, target, intent(inout) :: work
   integer, intent(out) :: st ! stat parameter
   ...
   blkm_new = nrow + nd
   size_bcol = (blkm_new+0_long) * blkn_new
   ...
   sa = sa + (blocks(dblk)%blkm+nd) * (blkn_new+0_long)
   if (blocks(dblk)%last_blk.gt.dblk) then
     blocks(dblk+1)%sa_new = sa
     sa = sa + blocks(dblk+1)%blkm * (blkn_new+0_long)
     do blk = dblk+2, blocks(dblk)%last_blk
       blocks(blk)%sa_new = sa
       sa = sa + blocks(blk)%blkm * (blkn_new+0_long)
     end do
   end if
   ...
   !
   if(.not. do_copy) then
      ! Need to zero upper triangular part to avoid innocous compiler messages
      ! NB: Following loop could be removed to maximize performance.
      do i = 1, blkn_new
         work((i-1_long)*blkm_new+1:(i-1_long)*blkm_new+blkn_new) = 0.0
      end do
   ...
   do i = 1,blkn
      ...
      ! variable errors.
      work(ip+1:ip+nd) = zero
      ip  = ip + nd
      work(ip+1:ip+nrow) = &
         lfact(bcol)%lcol(ip1+1:blkn*(nrow+0_long):blkn)
      ip  = ip + nrow
      ip1 = ip1 + 1
   end do
...
integer(long) function swizzle_sa(sa, blkn, nelim)
   integer(long), intent(in) :: sa
...
subroutine solve_task_dispatch(nbcol, lfact, stack, blocks, &
 ...
 !%%%  integer :: t_start, t_end
   ...
   real(wp), dimension(:), allocatable :: evars ! elimination variables
...
!$OMP FLUSH(stack,rhs)
      ...
      case(TASK_SLV_FSLV) ! Forwards solve with node
         ...
         do while(blk.le.nodes(node)%blk_en)
            ...
            call fwd_solve_bcol(bcol, blk, col, lfact, blocks, nrhs, &
               rhs, ldr, evars, ldr, xlocal, rhs_local,              &
!%%%           control,                                              &
               this_thread, total_threads)
         end do ! block column
      ...
      case(TASK_SLV_BSLV) ! Backward solve with node
         ...
         do while(blk.ge.nodes(node)%blk_sa)
            ...
            do r = 1, nrhs
               ...
               do i = 1, nelim
                  rhs_local(i+offset_local,0) = zero
               end do
            end do
            ...
            do blk = blk, blocks(blk)%dblk+1, -1
               ...
               idx_offset = int(blk - blocks(blk)%dblk) ! block row, 0 indexed
               idx_offset = 1 + s + idx_offset * nodes(node)%nb

               call slv_bwd_update(m, nelim, col, idx_offset, &
                  lfact(bcol)%index_new, &
                  lfact(bcol)%lcol(sa:sa+m*(nelim+0_long)-1), &
                  nelim, nrhs, rhs, rhs_local(:,0), ldr, xlocal &
!%%%              , control, blocks(blk)%id &
            ...
            if(m.gt.nelim) then
               ...
               call slv_bwd_update(m-nelim, nelim, col, nelim+1,           &
                  lfact(bcol)%index_new,                                   &
                  lfact(bcol)%lcol(sa:sa+(m-nelim+0_long)*nelim-1), nelim, &
                  nrhs, rhs, rhs_local(:,0), ldr, xlocal                   &
!%%%              , control, blocks(blk)%id                                &
                  )
            endif
...
subroutine fwd_solve_bcol( &
      bcol, blk, col, lfact, blocks, nrhs, rhs, ldr, &
      evars, lde, xlocal, rhs_local, &
!%%%  control, &
      this_thread, total_threads)
   ...
   real(wp), dimension(nrhs*ldr), intent(inout) :: rhs ! right hand side
      ! vector
   integer, intent(in) :: lde ! leading dimension of evars
   real(wp), dimension(nrhs*lde), intent(inout) :: evars ! workspace to
      ! store contigous version of rhs in (potentially shared)
   real(wp), dimension(*), intent(out) :: xlocal ! local workspace. Must be
      ! of size at least nrhs*nelim
   integer, intent(in) :: total_threads ! total number of threads running in
      ! parallel
   real(wp), dimension(nrhs*ldr, 0:total_threads-1), intent(inout) :: &
...
!%%%type(MA86_control), intent(in) :: control
   ...
   integer(long) :: sa ! start of block within bcol
   ...
   s     = n - blocks(blk)%blkn
   m     = blocks(blk)%blkm + s
   call slv_solve_fwd(nelim, nelim, col,                          &
      lfact(bcol)%lcol(1:nelim*(nelim+0_long)), nrhs, evars, lde  &
!%%%  , control, blocks(blk)%id                                   &
      )
   ...
   m = m - nelim
   if(m.gt.0) then
      sa = 1 + nelim*(nelim+0_long)
      call slv_fwd_update(m, nelim, col, idx_offset,              &
         lfact(bcol)%index_new,                                   &
         lfact(bcol)%lcol(sa:sa+m*(nelim+0_long)-1), nelim, nrhs, &
...
integer(long) function get_dest_block(src1, src2)
   ...
   sz = int(src1%last_blk - src1%dblk) + 1 
...
!
subroutine factor_solve_block(n, p, nb, s, a, la, buf, d, perm, &
   ...
   real(wp), intent(inout) :: a(la) ! holds trapezoidal matrix 
     ! to be factorized. Holds the factorization on exit.
      ! Delayed columns are permuted to the final columns. Held rowwise
      ! at all times.
   real (wp) :: buf(nb*(n+0_long)) ! Work array
   real (wp), intent (out) :: d(2*p) ! d(1:2*q) is set to hold the inverse
   ...
   !   flag = -7
   !else if (control%static < control%small .and. control%static/=zero) then
   !   flag = -10
   ...
   umin = min(max(control%umin,zero),u)
   ...
   if (s>0 .and. s<p) then
      ...
      do j = 1,i
         call swap_cols(n,nb,q,1,0,a,buf,perm,j,p+1-j)
      end do
   ...
   pivot: do
      ...
      sweep: do mdummy = 1, p-q ! Look for a pivotal column or pair of columns
         ...
         k = kkr1+m-rm-1
         if(q>rm) then
            call dgemv('NoTrans',n-m+1,q-rm,one,a(k),lq, &
               buf(n*(rm+0_long)+m),n,one,a(kkm),1)
         end if
         ...
         if(t>0 .or. m==p) then
            ...
            if (ubest1>u) then
               ...
               if (mbest/=m) &
                  call swap_cols(n,nb,q,1,rm,a,buf,perm,mbest,m)
               exit sweep
            end if
         end if
      end do sweep
      ...
      pivsiz0: if (pivsiz==0) then
         ...
         if (max(ubest1,ubest2)>=umin) then
            if (ubest1>=ubest2) then
               ...
               pivsiz = 1
               if (mbest/=m) &
                  call swap_cols(n,nb,q,1,rm,a,buf,perm,m,mbest)
            else
               ! Accept 2x2 pivot
               u = min(u,ubest2)
               ...
               detscale = detscale2
               t = min(mbest1,mbest2)
               if (t/=q+1) call swap_cols(n,nb,q,1,rm,a,buf,perm,q+1,t)
               t = max(mbest1,mbest2)
               if (t/=q+2) call swap_cols(n,nb,q,1,rm,a,buf,perm,q+2,t)
               t = q+1
            end if
      ...
      pivsizes: if (pivsiz==1) then
         ...
         if (q+1/=m) call swap_cols(n,nb,q,1,rm,a,buf,perm,q+1,m)
         ! We store D**-1.
         ...
         if (a(kkq1)<zero) tstats%num_neg = tstats%num_neg + 1
         ...
         !end do
         pivval = d(2*q+1)
         do ilong = kq1+1, kq1+n-q-1
            buf(ilong) = -a(ilong)
            a(ilong) = pivval*a(ilong)
         end do
         ...
         if (q+2/=m) then
            if (q+1/=t) call swap_cols(n,nb,q,1,rm,a,buf,perm,q+1,t)
            call swap_cols(n,nb,q,1,rm,a,buf,perm,q+2,m)
         end if
         ! We store D**-1
         k = kkq1 + lq + 1
         ...
         if(m>=j) then
            call dgemm('n','t',n-j+1,m-j+1,2,one,a(kkq1+j-q-1), &
               n-((q+1)/nb)*nb,buf(kq1+j-q-1),n,one,a(kkj),lq)
         end if
         ! Update q and kkq1
         kkq1 = k + lq + 1
         q = q + 2
...
!
! Copy from rowwise a_in to colwise a_out
!
subroutine row_to_col(n, nb, a_in, a_out)
   integer, intent(in) :: n
   integer, intent(in) :: nb
   real(wp), dimension(n*(nb+0_long)), intent(in) :: a_in
   real(wp), dimension(n*(nb+0_long)), intent(inout) :: a_out ! inout to allow
      ! pre-zeroing of this array without it being randomly initialised.
      ! This only causes problems for the NAG compiler's highest checking mode
      ! and is almost surely innocous anyway (as it only affects the upper
      ! triangular enties that we don't actually ever use)
   ...
   do x = xts*(nb/xts)+1, nb
      i = x
      j = 1+(x-1)*n
      call dcopy(n, a_in(i), nb, a_out(j), 1)
   end do
...
subroutine col_to_row(n, nb, lfact, colwork, st)
   ...
   real(wp), dimension(*), intent(in) :: colwork
   integer, intent(out) :: st
   ...
   if(allocated(lfact%lcol)) then
      if(size(lfact%lcol) < (n+0_long)*lfact%blkn_new) then
         deallocate(lfact%lcol, stat=st)
         allocate(lfact%lcol( (n+0_long)*lfact%blkn_new ), stat=st)
         if(st.ne.0) return
      endif
   else
      allocate(lfact%lcol( (n+0_long)*lfact%blkn_new ), stat=st)
   ...
   k = 1
   do j = 1, nb
      call dcopy(n-j+1, colwork(k), 1, lfact%lcol(i), nb)
      i = i + nb + 1
      k = k + n + 1
   end do

   ! Copy delays to end of lcol
   lfact%lcol((n+0_long)*nb+1:(n+0_long)*lfact%blkn_new) = &
      colwork((n+0_long)*nb+1:(n+0_long)*lfact%blkn_new)
...
! dest <- dest - src1^T src2
! Remember that the blocks are stored by rows so that
! src1 holds A_ij by rows
! src2 holds A_kj^T by cols
! dest, src1 and src2 all belong to the same node.
!
subroutine update_block_block(m, n, nelim, dest, blk, n1, src1, src2, &
      srcd, ldwork &
!%%%  , control &
      )
   ...
   real(wp), dimension(*), intent(in) :: srcd ! inverse of diagonal
   real(wp), dimension(*), intent(out) :: ldwork ! work array for  
      ! calculating LD
!%%%type(MA86_control), intent(in) :: control
...
!%%%integer :: t_start, t_end, this_thread
   ...
   call calc_ld(n, nelim, n1, src1, srcd, ldwork)
...
!$ call omp_set_lock(blk%alock)
   ...
   call dgemm('T', 'N', n, m, nelim, -one, ldwork, nelim, src2, n1, coeff, &
      dest, n)
...
!
! Subroutine takes a block of L (stored by rows) and D^-1, calculates the
! corresponding block of LD (stored by rows)
!
subroutine calc_ld(m, n, ldl, l_mat, d_vec, ld_mat)
   ...
   real(wp), dimension(ldl*m), intent(in) :: l_mat
   real(wp), dimension(2*n), intent(in) :: d_vec
   real(wp), dimension(m*n), intent(out) :: ld_mat
   ...
   real(wp) :: d1, d2, d3
   real(wp) :: det
   real(wp), dimension(2*n) :: dalt ! temporary array storing calculated
      ! values of D
   ...
   do while(col.le.n)
      if(d_vec(2*col).eq.zero) then 
         ...
         if(d_vec(2*col-1).eq.zero) then
            ...
            dalt(2*col-1) = one/d_vec(2*col-1)
         endif
         col   = col   + 1
      else 
         ! 2x2 pivot
...
!
subroutine update_between(dest, dnode, n, nelim, sindex, lcol, lcol_src,   &
      d_src, blocks, col_list, row_list, buffer, control, info,            &
      st, ldwork                                                           &
!%%%  , scol, snode                                                        &
      )
   ...
   type(block_type), dimension(*), intent(inout) :: blocks
   real(wp), dimension(*), intent(inout) :: lcol ! holds  block
      ! column of factor L that dest belongs to
   real(wp), dimension(*), intent(in) :: lcol_src ! holds  block
      ! column of factor L that source block belongs to
   real(wp), dimension(*), intent(in) :: d_src ! holds inverse of
   ...
   real(wp), dimension(*), intent(out) :: buffer ! work array
   type(MA86_control), intent(in) :: control
   integer, intent(inout) :: info   
   integer, intent(inout) :: st
   real(wp), dimension(:), allocatable, intent(inout) :: ldwork ! work array
      ! for calculating LD
...
!%%%if(control%unit_log.gt.0) call system_clock(t_start)
   ...
   i = dcol + int(blocks(dest)%id - blocks(dest)%dblk) ! block in snode
   ...
   i = int(dest - blocks(dest)%dblk) + 1 ! row block of dest column
   ...
   ! are stored by rows)
   ilong = 1 + nelim*(s2sa - 1_long) ! Start of rsrc
   jlong = 1 + nelim*(s1sa - 1_long) ! Start of csrc
   ...
   call calc_ld(s1en-s1sa+1, nelim, nelim, lcol_src(jlong), d_src, ldwork)

   if(n.ge.control%min_width_blas) then
      ! High flop/buffer sz ratio => perform operations into buffer with BLAS

      ndiag = 0
      call dgemm('T', 'N', int(s1en-s1sa+1), int(s2en-s2sa+1), nelim,      &
         -one, ldwork, nelim, lcol_src(ilong), nelim, zero, buffer, col_list_sz)
...
!$    call omp_set_lock(blocks(dest)%alock)
      ...
      if(.not.blocks(dest)%touched) then
         ! Zero block
         lcol( blocks(dest)%sa : &
            blocks(dest)%sa+blocks(dest)%blkn*blocks(dest)%blkm-1 ) = zero
...
!$    call omp_set_lock(blocks(dest)%alock)
      ...
      call update_direct(lcol, lcol_src, ldwork, blocks(dest)%sa,              &
         blocks(dest)%blkn, 1_long, ilong, nelim, nelim, row_list, row_list_sz,&
...
!
subroutine update_direct(lcol, lcol_src, lcold_src, dsa, blkn, s1sa, s2sa, &
   ...
   real(wp), dimension(*), intent(in) :: lcold_src ! holds block
      ! column of factor LD that the source blocks belong to
   integer(long), intent(in) :: dsa ! posn of the first entry of the
      ! destination block within lcol
   integer, intent(in) :: blkn ! number of columns in lcol
   integer(long), intent(in) :: s1sa ! points to first row in source block csrc
   integer(long), intent(in) :: s2sa ! points to first row in source block rsrc
   ...
   do j = 1, ndiag
      cptr = dsa + (row_list(j)-1_long) * blkn - 1
      rptr = s2sa + (j-1_long) * srcn
      do i = 1, j
         k1 = s1sa + (i-1_long) * nelim
         work1 = zero
   ...
   do j = ndiag+1, rls
      ...
      i = 4*int(cls/4)
      do i = 1, i, 4
         k1 = s1sa + (i+0-1_long) * nelim
         k2 = s1sa + (i+1-1_long) * nelim
         k3 = s1sa + (i+2-1_long) * nelim
         k4 = s1sa + (i+3-1_long) * nelim
...
subroutine expand_buffer(a, blkn, row_list, rls, col_list, cls, ndiag, buffer)
   ...
   rptr = 1
   do j = 1, ndiag
      cptr = 1 + (row_list(j)-1_long)*blkn - 1
      do i = 1, j
         k = cptr + col_list(i)
         a(k) = a(k) + buffer(rptr)
         rptr = rptr + 1
...
subroutine ma64_update(n,nb,kkq,a,buf,jl,jr,pl,pr)
   ...
   do i = 1, j2-j+1, bs
      ...
      call dgemm ('n', 't', n-j+1-i+1, p, pr-pl+1, one, &
         a(kkq+j-1+n*(pl-1_long)+i-1), n, buf(j+n*(pl-1_long)+i-1), n, &
         one, a(kkj), n)
   end do
...
! This was based on a routine within ma64.
!
subroutine swap_cols(n,nb,q,q1,rm,a,buf,perm,j1,j2)
   integer, intent(in) :: n
   integer, intent(in) :: nb
   integer, intent(in) :: q
   ...
   call dswap(q-q1+1-rm,buf(j1+(rm+0_long)*n),n,buf(j2+(rm+0_long)*n),n)
   ...
   l = j1-1
   if (l>0) call dswap(l,a(k1),n,a(k2),n)
   ...
   jb = j2-j1-1
   l = min(jb,nb-j1,j2-1)
   if (l>0.and.j2.ge.1) call dswap(l,a(k1),1,a(k2),n)
...
!
subroutine solve_add_D(q,d,rhs,rhs_local,index)
   integer, intent (in) :: q ! Order of L (and D)
   real (wp), intent (in) :: d(2*q) !  Holds the inverse of D,
      ! except that zero diagonal blocks (pivots) are not inverted.
      ! Diagonal entries are in d(1:2*q-1:2) and entries to the right of
   ...
   do while(j.le.q)
      ...
      if (d(2*j)==zero) then
         ...
         ! 2x2 pivot
         i2 = index(j+1)
         rhs_local(j)   = rhs_local(j)   + rhs(i)*d(2*j-1) + rhs(i2)*d(2*j)
...
! B_j <- L_jj^-1 B_j
!
! L_jj is stored by rows so we have to take transpose in blas calls
!
...
! slv_bwd_update
!
subroutine slv_solve_fwd(n, nelim, col, dest, nrhs, rhs, ldr &
!%%%  , control, id &
      )
   ...
   real(wp), dimension(n*nelim), intent(in) :: dest ! holds L_jj
   integer, intent(in) :: nrhs ! number of right-hand sides
   integer, intent(in) :: ldr ! leading extent of rhs
...
!%%%if(control%unit_log.gt.0) call system_clock(t_start)
   ...
   if(nrhs.eq.1) then
      call dtrsv('Upper', 'Tranpose', 'Unit', nelim, dest, n, rhs(col), 1)
   else
      call dtrsm('Left', 'Upper', 'Transpose', 'Unit', nelim, nrhs, one, &
         dest, n, rhs(col), ldr)
   endif
...
! slv_bwd_update
!
subroutine slv_solve_bwd(n, nelim, col, dest, nrhs, rhs, ldr &
!%%%  , control, id &
      )
   integer, intent(in) :: n ! leading dimension of L_jj
   integer, intent(in) :: nelim ! number eliminations (immediate return if =0)
...
!%%%if(control%unit_log.gt.0) call system_clock(t_start)
   ...
   if(nrhs.eq.1) then
      call dtrsv('Upper', 'N', 'Unit', nelim, dest, n, rhs(col), 1)
   else
      call dtrsm('Left', 'Upper', 'N', 'Unit', nelim, nrhs, one, &
         dest, n, rhs(col), ldr)
   endif
...
!
subroutine slv_fwd_update(m, nelim, col, offset, index, dest, ldd, &
      nrhs, upd, ldu, rhs, ldr, xlocal &
!%%%  , control, id &
      )
   integer, intent(in) :: m ! number of rows in block
   integer, intent(in) :: nelim ! number eliminations (immediate return if =0)
   ...
   real(wp), dimension(ldd*m), intent(in) :: dest ! holds L_ij
