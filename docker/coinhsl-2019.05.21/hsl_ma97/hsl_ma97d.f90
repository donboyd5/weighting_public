...
!$OMP ATOMIC
   ...
   10 continue
   ! Memory allocation failed either in this subroutine (including by calls to
   ! smalloc and stack_alloc) or in ldsrk. We have to deallocate the local
   ! stacks explicitly because they are stored as linked lists. If a stack has
   ! not been allocated, then the pointer is NULL and stack_free_all will do
   ! nothing.
   call stack_free_all(stack_mem_odd)
   call stack_free_all(stack_mem_even)
   stats(this_thread)%flag = MA97_ERROR_ALLOCATION
...
!$ this_thread = omp_get_thread_num() + 1
   ...
   10 continue
   ! Memory allocation failed (st /= 0). Deallocate local stacks.
   call stack_free_all(stack_mem_odd)
   call stack_free_all(stack_mem_even)
